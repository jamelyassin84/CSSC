import { yearLevels } from './AppConstants'
...
export const GovProcesses = (govs: any, departments: any) => {
	...
	govs.forEach((candidate: any) => {
		for (let key in obj) {
			if (candidate.votes === obj[key].votes) {
				...
				if (
					obj[key].votes === undefined ||
					candidate.votes > obj[key].votes
				) {
	...
	for (let key in obj) {
		if (Array.isArray(obj[key])) {
			obj[key] = sortBy(obj[key], 'voter.name', 'DESC', 'string')
		...
		for (let key in obj) {
			if (candidate.votes === obj[key].votes) {
				...
				if (
					...
					const year = string[1]
					if (
						candidate.voter.department === department &&
						candidate.voter.year === year
					) {
						obj[key] = candidate
